            ...
            $.each(data, function(key, value) {
              //search bias
                if (value.manufacture.search(expression) != -1 || value.Model.search(expression) != -1)
                {
                  ...
                  $('#result').append('<li class="list-group-item link-class"><button formation=" '+value.manuelurl+' " type="button"  id="'+value.Model+'" class="btn btn-secondary btn-lg btn-block" aria-pressed="true">'+value.manufacture+' | '+value.Model+'</button></li>');
